...
#define defaultSyntax         "./list -i in.txt -o out.txt [-d (del duplicates) -v (verbose) -m merge.txt (merge) -r abc (removes)]\n"
...
gint main (gint argc, gchar *argv[])
{
  ...
  while ((ch = getopt(argc, argv, "i:m:o:vdr:")) != -1)
  {
    switch (ch) {
       ...
       */
        ...
        if (merge_file == NULL) {
          fprintf(stderr, defaultErrorSyntax "merge file not specified\n program terminated\n", argv[0]);
          if (input_file) {
            fclose(input_file);
      ...
      case 'o':
        ...
        if (output_file == NULL) {
          fprintf(stderr, defaultErrorSyntax "output file not specified\n program terminated\n", argv[0]);
        if (input_file) {
          g_free(input_filepath);
          fclose(input_file);
        }
      ...
      case 'r':
        if(optarg == NULL) {
          fprintf(stderr, defaultErrorSyntax "name not specified\n program terminated\n", argv[0]);
          return EXIT_FAILURE;
        }
  ...
  // There has to be a pipe/file on stdin, or input_file has to be set, to run the program
  if (!isatty(fileno(stdin))) {
    input_file = stdin;
  }
  else {
